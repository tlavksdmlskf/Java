...
public class ClassMetaMain {
    public static void main(String[] args) throws Exception{
        ...
        for (Class i : interfaces){
            System.out.println("interfase: " + i.getName());
        }
    ...
    2. 리플렉션: 클래스에 정의된 메서드, 필드, 생성자 등을 조회하고, 이들을 통해 ㅐㄱ체 인스턴스를 생성하거나
    메서드를 호출하는 등의 작업을 할 수 있다
    3. 동적 로딩과 생성 : Class.forName() 메서드를 사용하여 클래스를 동적으로 로드하고, newInstance() 메서드를 통해
    새로운 인스턴스를 생성할 수 있다
    참고(동적 로딩 ) - 런타임에 클래스로 로드하는 프로세스를 말함
    4. 애노테이션 처리 : 클래스에 적용된 애노테이션을 조회하고 처리하는 기능을 제공한다

     class의 주요 기능
     1. getDeclaredFields() : 클래스의 모든 필드 조회
     2. getDeclaredMethods() : 클래스의 모든 메서드를 조회한다
     3. getSuperclass() ㅣ 클래스의 부모 클래스를 조회한다.
