...
public class WrapperClassMain {
    public static void main(String[] args) {
        ...
        System.out.println("inttegerObj = " + integerObj);
...
/*
래퍼 클래스 생성 - 박싱(Boxing)
- 기본형을 래퍼 클래스로 변경하는 것을 마치 박스에 물건을 넣는것 같다고 해서 박싱이라 한다.
...
-추가로 Integer.valueOf()에는 성능 최적화 기능이 있다.
  ...
  - 해당 범위의 값을 조회하면 미리 생성된 Integer 클래스를 미리 생성해준다다.
  - 해당 범위의 값이 없으면 new Integer()를 호출한다
    - 마치 문자열 풀과 비슷하게 자주 사용하는 숫자를 미리 생성해두고 재사용한다

intValue() - 언박싱
- 래퍼 클래스에 들어있는 기본형 값을 다시 꺼내는 매서드이다.
- 박스에 들어있는 물건을 꺼내는 것 같다고 해서 언박싱이라 한다

비교는 equals() 사용
- 래퍼 클래스는 객체이기 때문 == 비겨를 하면 인스턴스의 참조값을 비교한다.
